        ...
        for table in tables:
            ...
            for row in rows:
                ...
                if first_td and first_td.find('a'):
                    ipo_name = first_td.get_text(strip=True)  # Extract the IPO name (text in <td>)
...
def get_urls_by_names(name, data_list):  
    ...
    cleaned_name = re.sub(r'\b(ltd|pvt|industries|solutions|international|technologies)\b', '', name, flags=re.IGNORECASE).strip().lower()
